...
static void		copy_source_in_img(t_data *data, struct s_png_ihdr *png_ihdr,
	int id_img, uint8_t *idata)
{
	size_t		i_source;
	size_t		i_dest;
	uint32_t	transp;
	...
	i_source = 0;
	i_dest = 0;
	while (i_dest < png_ihdr->width * png_ihdr->height)
	{
		if (i_source % (png_ihdr->width * png_ihdr->bpp + 1) == 0)
			i_source++;
		if (png_ihdr->bpp == 4)
			transp = 255 - idata[i_source + 3];
		else
			transp = 0;
		if (png_ihdr->bpp == 3 || png_ihdr->bpp == 4)
			data->imgs[id_img].addr[i_dest] = get_color_code(idata[i_source],
				idata[i_source + 1], idata[i_source + 2], transp);
		else
			data->imgs[id_img].addr[i_dest] = get_color_code(idata[i_source],
				idata[i_source], idata[i_source], transp);
		i_source += png_ihdr->bpp;
		i_dest++;
	}
}
